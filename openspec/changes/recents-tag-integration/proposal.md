...
- Add folder tag editing to Library Files: the "Edit tags" folder-row action, previously a permanently-disabled stub, now opens the same shared tag editor sheet tracks/loops/playlists already use. **Scope change from the original proposal** (confirmed with user): folder tag editing was originally a non-goal, since this change was meant to only add ways to browse/play by tags that already exist; it's now in scope because folders were the only one of the four taggable entity types with no live tag-write path, which also left `1.6`'s per-tag add-date stamping for folders unreachable through the UI. See design.md's "Folder tag editing is added..." decision.
