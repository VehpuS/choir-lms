...
**Alternative considered:** one chip per active filter (Show + each tag), each individually removable without opening the popover. Rejected per explicit confirmation — more UI surface and a new per-chip removal interaction, for a proposal whose goal is *visibility* of existing state, not a new way to mutate it. If demand for inline removal shows up later, that is a natural, separately-scoped follow-up.
...
- [The summary chip and dot indicator are global (`entityFilter`/`selectedTagFilters` state), so a filter set while in Files stays visibly indicated after switching to Tracks/Loops/Playlists/Tags, even in views with no `Show` UI of their own] → Acceptable, and intentional: this is the correct complement to `library-views-sort-search-parity`'s entity-filter-reset fix — that fix stops a stale Files-only filter from *silently* suppressing results elsewhere; this proposal makes any filter that *is* still active *visible* everywhere it can take effect, rather than only where it was set.
