...
Two things arrive together. First, the mobile rehearsal player's visual layer is being moved onto the Nocturne design system (dark near-neutral ground, Inter at medium weight, 8px radii, a single blurple accent used as a line and a glow rather than a flood) and onto mainstream music-player list conventions — rows instead of stacked cards, one large title per destination, transport where a singer's thumb already rests. Design artifacts exist for ten surfaces (screens 1a–1j, see `reference-images/`). Surfaces that shipped after the mockups were drawn — Drive search selection mode, the Drive import review / progress / completion screen, and the saved-track original-location actions — have no companion mockup and are specified to inherit the same primitives. No feature is removed and no destination moves: the three-tab shell, the five Library views, the explorer, the mini-player, the bottom-sheet playback surface, and the queue all keep their current behavior.

Second, the redesign work surfaced a rehearsal capability the app does not have and that the visual layer needs a home for: **playback shaping**. Singers learn a hard passage slowly and, when their part sits at the edge of their range, at a different pitch. Today neither is possible. Speed and pitch also cannot be a purely transient control, because the settings a singer arrives at for a passage are exactly the thing they want back next rehearsal — so each is specified both as an ephemeral session option and as an input to creating an adjusted track or loop in the library.
...
- Surfaces without a mockup are restyled by inheritance, with no behavior change: Drive search selection mode (selection toolbar, per-row selected state, `Select All Matching`, `Continue` / `Edit Selection`), mixed folder-and-audio search results with their containing-path meta line and the `Search results` return action, the Drive import review screen (destination picker, `Preserve structure` / `Flatten`, summary counts, phased progress, completion summary with failed-item reasons and `Retry failed`), the saved-track `Show in Add` / `Open in Google Drive` actions with their `Checking Drive…` pending state and `From <path>` provenance line, and the playlist-detail ordered / shuffle action row. Their filled primary buttons become accent outlines like every other primary action.
...
- Committing the current shaping settings creates an **adjusted track** or **adjusted loop**: a first-class library entity that references its source and carries its transform, so the settings come back next rehearsal and can be tagged, queued, added to playlists, and searched like any other entity.
...
- `playback-shaping`: speed and pitch as both ephemeral playback options and adjusted-entity creation inputs, with tempo source as an extension point.

### Modified Capabilities

- `mobile-rehearsal-player-ui`: gains a Nocturne visual-system requirement (ground, ramps, type, radii, spacing, accent-as-line, Phosphor icons, row-based lists); the dedicated-playback-screen requirement gains scenarios for the collapsed/expanded two-state split, for the waveform rendering from real analysis, and for reaching the shaping surface; the mini-player requirement gains scenarios for real-peak rendering and the active-shaping context line; a new requirement covers surfaces with no companion mockup (Drive search selection, Drive import review/progress/completion, original-location actions, playlist-detail play actions). The pitch-lock prohibition lives in `playback-shaping` (it spans every surface, not just icons), and the existing icon-semantics scenarios are re-verified against Phosphor rather than rewritten.
- `practice-loops-and-playlists`: saved loops and adjusted entities are both usable wherever a saved track or loop is usable (queue, playlists, tags, search).
- `mobile-library-organization`: adjusted entities appear in the Files/Tracks/Loops views, sort, and search results, are visually distinguishable from their source, and are excluded from bulk-import source reuse; the track-level remove requirement is reworded ("loops of that track") and extended so removing a source track also removes its adjusted entities.

### Explicitly Unchanged

The advanced search, filter, and tag features stay exactly as specified today — app-owned dedicated search, folder vs whole-library scope, entity-type filters, explicit sort field and direction, tag filters with Any/All match mode, the tag editor's suggestion row, Recents' popular tags, and the Tags view with usage counts. This change restyles those controls and does not reduce, merge, or relocate any of them. `mobile-library-organization`'s search/filter/sort requirements and all of `recents-tag-navigation` are untouched except for adjusted entities appearing in results.

The same holds for everything shipped by `improve-drive-search-and-bulk-library-import` and `mobile-rehearsal-ux-audit-fixes` (both archived 2026-09-23 after this change was first drafted): folder-aware paginated Drive search with path context and the `Search results` return action, search-result selection and `Select all matching`, recursive import with `Preserve structure` / `Flatten`, import review, progress, cancellation, partial-failure retry, original-folder provenance and `Show in Add` / `Open in Google Drive`, save acknowledgment that stays non-stale, the Files create control that never obscures the last row, playlist-detail ordered and shuffle actions, and the repeat-vs-shuffle and drag-handle distinctness scenarios. The requirements in `mobile-practice-library` and `mobile-rehearsal-player-usability` are untouched; this change restyles their surfaces only.

## Impact

- Code: `src/app/utils/theme.ts` (token rewrite), every `styles.ts` / `StyleSheet.create` block under `src/app/**` that reads `appTheme` or hard-codes a hex, the shell (`src/app/routing/shell/**`), the playback surface (`src/app/routing/playback/**`), the queue (`src/app/routing/queue/**`), Library and Add screens and their row/explorer components, the Drive import review screen (`src/app/screens/drive-import-review/**`), the Drive search selection and folder-result rows (`src/app/library/drive/components/**`), the saved-track options menu and original-location feedback (`src/app/library/components/options-menu-sheet/**`, `saved-rehearsal-library-section/**`), and the icon layer (MaterialCommunityIcons → Phosphor; 24 files import `@expo/vector-icons` today).
- New code: a playback-shaping model and hook, a shaping surface, adjusted-entity persistence in `@org/audio-library-models` / `audio-library-runtime` and `local-library-storage`, and waveform peak extraction replacing the stub in `src/app/components/playback-waveform/model.ts`.
...
- Persisted data: adjusted entities add a stored shape. `design.md` proposes transform metadata rather than rendered audio, which keeps the change compatible with the existing by-reference Drive model and the "no offline playback" MVP boundary.
- Design artifacts: `reference-images/1a…1j.png` in this change directory are the canonical companion states referenced by the spec deltas; `design-source/Rehearsal Player Redesign.dc.html` is their source.
...
- No change to Drive auth, Drive browsing/search/selection/import behavior, source provenance, or the by-reference save model.
