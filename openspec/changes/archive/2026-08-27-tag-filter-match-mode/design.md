...
### Toggle control: a two-segment switch in the `TAGS` group header, via a new `TagFilterMatchModeSwitch`

Add a small trailing control to the existing `TAGS` group block in `library-search-filter-popover.tsx` (currently a bare `Text` label + wrapped tag chips), giving that block the same `filterLabelRow` (label + trailing control) shape `FilterChipGroup` already uses for `Sort`'s ascending/descending toggle. The control is a new `TagFilterMatchModeSwitch` component: a compact two-segment switch showing both `All` and `Any` labels side by side inside one bordered pill track, with the active segment given the app's standard "selected" dark treatment (`INTERACTION_CHIP_TOKENS.selectedBackground`/`selectedText`) and the inactive segment left neutral. Tapping either segment calls `onSelectMatchMode(value)` directly (not a flip), matching the `onSelectValue`-style interaction `FilterChipGroup` already uses for `Scope`/`Sort`/`Show`.

**Superseded decision:** the original design reused `InteractionChip` as a single tap-to-flip pill reading only the current state (`Match: All` / `Match: Any`). Rejected after implementation review: a single passive-styled chip reads as inert/unselected at a glance (it never took the app's dark "selected" treatment used everywhere else for an active, non-default choice), and it's visually near-identical to the entity-type/tag chips beside it, giving no cue that it's a mode switch for the section above rather than another filterable option. A two-segment switch that always shows both named states, with the active one visually distinguished the same way every other "selected" control in this popover already is, reads unambiguously as a switch and needs no separate label-resolution helper (`resolveTagFilterMatchModeToggleLabel` was removed as a result — both segment labels are now static).

**Alternative considered:** an icon-only toggle mirroring `SurfaceIconButton`'s sort-direction arrows exactly. Rejected — there is no widely-recognized icon pair for "match all" vs "match any" the way ascending/descending arrows are self-explanatory; showing both words is clearer here and costs no more space than the existing tag chips it sits beside.
