...
- The header's filter toggle button gains a small active-filter dot indicator that stays visible whenever `Show`/`Tags` filters are non-default (`hasActiveFilters`), independent of whether the filter popover itself is open or closed. As part of the same change, the filter toggle button's filled (white background) state now reflects only whether the filter popover is currently open, no longer also filling when a filter is merely active — keeping the open/closed fill state visually unambiguous now that the dot and the summary chip below already carry the active-filter signal.
