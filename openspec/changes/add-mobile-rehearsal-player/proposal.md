...
- Allow users to add saved full tracks and saved loops into playlists through track context menus and playlist selector overlays, persist explicit ordered playlist-entry relationships between rehearsal items and playlists, and support ordered playback, repeat, and shuffle.
- Define a music-app-inspired mobile UI shell for discovery, search, personal library management, playlists, and playback queues that uses sliders for loop and playback adjustments plus a consistent music-control icon vocabulary so the app feels closer to a dedicated listening product than a utility workflow.
