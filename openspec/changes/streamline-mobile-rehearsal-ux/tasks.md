...
- [ ] 5.3 Add a unified `Files` Library view that manages saved entities as file-like items within a folder hierarchy, including loops, as part of the first organization baseline.
- [ ] 5.3.1 Ensure folders can contain loops directly and the Files view or folder results surface those loops without requiring navigation through the parent track first.
- [x] 5.3.2 Add a first-class Library view switcher that exposes the unified `Files` view alongside dedicated entity views that preserve current focused browsing UX.
- [ ] 5.3.3 Keep dedicated Tracks, Loops, and Playlists views consistent with current row/card interactions while allowing Files-driven organization to coexist.
...
- [ ] 6.2.7 Manually verify loop navigation and organization intent: the Library view switcher exposes Files alongside dedicated Tracks/Loops/Playlists views; Files shows mixed-entity folder content; the top-level Saved loops section remains available in focused browsing; tracks with loops expose `View track loops`; the track-scoped loop view keeps parent-track context visible, supports ordered loop-series playback plus row-start playback, surfaces a `Make new loop` action for that track, keeps add-to-playlist, queue, and remove actions easy to reach; loops remain independently taggable and folder-addressable; search/tag/folder contexts show loops in their own visible result category with parent linkage; and `Make loop` remains a saved-track-only entry point.
