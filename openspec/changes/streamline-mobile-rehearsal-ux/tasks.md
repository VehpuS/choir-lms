...
- [x] 4.2.9 Add a `Create new playlist` action in the Up Next queue summary area to save the current queue order as a new playlist while preserving uninterrupted playback.
- [x] 4.2.10 Add an `Update playlist` action in the Up Next queue summary area to update an existing playlist with currently enqueued items in queue order, without mutating the active queue session.
...
- [ ] 4.4.1 Replace the default flat Saved loops Library section with a `View track loops` saved-track overflow action that opens a track-scoped loop surface when loops exist, while preserving loop action parity for playback, add to playlist, queue, and remove flows.
- [ ] 4.4.2 Keep the track-scoped loop surface visibly tied to its parent track while reusing the same shared loop row actions and metadata needed for fast playback and playlist workflows.
...
- [ ] 6.1.6 Add focused automated coverage for `View track loops` availability, track-scoped loop action parity, and loop result-category behavior across search, tag, and folder contexts.
...
- [ ] 6.2.2 Manually verify queue-to-playlist capture from Up Next: use `Create new playlist` and `Update playlist`, confirm resulting playlist item order, and confirm current playback item and position remain uninterrupted.
