...
- [ ] 2.7.12 Reduce default playlist-detail row management density. Keep direct play available, but consider moving drag handles and up/down controls behind an explicit reorder affordance or visually quieting them so playlist detail reads as a listening surface before it reads as an editor.
- [ ] 2.7.13 Clarify Now Playing repeat controls by presenting repeat state as one mode control or a clearly selected segmented control instead of separate adjacent `Repeat off` and `Repeat one` actions that can read as independent buttons.
- [ ] 2.7.14 Add destination context to Files-launched playlist creation dialogs. When the create flow starts from a folder, show the target folder in the dialog title or secondary line, for example `Create a playlist in Browser Test Folder`, while preserving the generic Library Playlists create dialog outside Files.
