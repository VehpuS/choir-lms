...
- [ ] 6.2.3.1 Manually verify playlist edit `Move to position` behavior: open playlist edit mode, invoke `Move to position`, confirm the slider is bounded from playlist position `1` through the last playlist position, move an item to first, middle, and last positions, and confirm the order updates without leaving edit mode or interrupting active playback.
