...
- [ ] 2.5.9 Improve folder row metadata so mixed-content folders are easier to scan in music-library browsing. Replace or augment generic item counts with compact type-aware summaries such as track, loop, and playlist counts while preserving a concise fallback for narrow rows and unknown item mixes.
