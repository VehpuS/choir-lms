...
- Applications → `apps/<name>`. Check the root `package.json` `workspaces` array for an existing `apps/*` entry.
...
`nx import` can add the imported directory itself (e.g. `apps`) to the root `workspaces` array, **NOT** glob patterns for packages within it. Cross-package imports will fail with `Cannot find module`.
...
- **Subdirectory import**: Plugins NOT auto-detected. Manually add with `npm exec -- nx add @nx/PLUGIN`. Check `include`/`exclude` patterns — defaults won't match alternate directories (e.g. `apps-beta/`).
- Run `npm exec -- nx reset` after any plugin config changes.

### Redundant Root Files (Whole-Repo Only)

Whole-repo import brings ALL source root files into the dest subdirectory. Clean up:

- `package-lock.json` — stale; dest has its own lockfile
...
3. Then `npm exec -- nx add @nx/eslint` to register the plugin in `nx.json`

Install `typescript-eslint` explicitly — do not rely on transitive workspace installs for it.
...
`@nx/eslint` may peer-depend on ESLint 8, causing the wrong version to resolve. If lint fails with `Cannot read properties of undefined (reading 'allow')`, add a root `overrides` entry:
...
After import, compare key deps (`typescript`, `eslint`, framework-specific). If dest uses newer versions, upgrade imported packages to match (usually safe). If source is newer, may need to upgrade dest first. Use root `overrides` to enforce single-version policy if desired.
...
Same `name` in `package.json` across source and dest causes `MultipleProjectsWithSameNameError`. **Fix**: Rename conflicting names (e.g. `@org/api` → `@org/teama-api`), update all dep references and import statements, `npm install`. The root `package.json` of each imported repo also becomes a project — rename those too.

### Workspace Dep Import Ordering

`npm install` can fail during `nx import` if a local workspace dependency has not been imported yet. File operations still succeed. **Fix**: Import all projects first, then `npm install`.
...
1. Run `npm exec -- nx add @nx/jest` — registers `@nx/jest/plugin` in `nx.json` and updates `namedInputs`
...
`nx import` may bring `node_modules/` and `package-lock.json` from the source. Both are stale.

**Fix**: `rm -rf imported/node_modules imported/package-lock.json imported/.gitignore`, then `npm install`.
