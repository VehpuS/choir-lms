...
Next.js detects missing `@types/react` during `next build` and may try to install it automatically with a package-manager-specific command. In this npm workspace, that can fail outright or dirty the repo with the wrong install flow.
...
- Errors found & fixed:
  1. Root `workspaces`: `apps`/`libs` → `apps/*`/`libs/*`
  ...
  5. Next.js attempting an automatic `@types/react` install — fixed by installing at root
...
- Errors found & fixed:
  1. Root `workspaces`: `apps/web` → `apps/*`
  2. Stale files: `node_modules/`, `package-lock.json`, `.gitignore` — deleted
...
- Errors found & fixed:
  ...
  2. Stale files from Vite source: `node_modules/`, `package-lock.json`, `.gitignore`, `nx.json`
  3. Removed rewritten scripts from Vite app's `package.json`
  4. ESLint 8 vs 9 conflict — `@nx/eslint` peer on ESLint 8 resolved wrong version. Fixed with root `overrides`
