...
React 18 (source) + React 19 (dest): the workspace may resolve mismatched `react-dom`, causing `TypeError: Cannot read properties of undefined (reading 'S')`. **Fix**: Align versions with root `overrides`.
...
3. Then `npm exec -- nx add @nx/eslint`
...
`vue-eslint-parser` must be an explicit dependency; do not rely on transitive installs for it.
