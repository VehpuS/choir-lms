...
- Tests: Prefer colocated `*.spec.ts` or `*.spec.tsx` files for new coverage under `src/**`. If moved or new runtime tests should run in `mobile-rehearsal-player:test`, update that target in `packages/mobile-rehearsal-player/package.json` because it currently enumerates files manually.
