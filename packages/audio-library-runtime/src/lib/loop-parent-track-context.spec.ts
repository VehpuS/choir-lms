...
describe('AsyncStoragePracticeRepository loop parent-track context', () => {
  it('repairs or preserves loop parent-track context when loops are read back from storage', async () => {
      ...
      [
        ...
        JSON.stringify([
          ...
          },
          {
            ...SAVED_LOOP,
            id: 'loop-3',
            name: 'Missing parent-track context loop',
            sourceId: 'drive:missing-source',
            sourceName: '',
          },
        ]),
  ...
  it('rejects saving a loop when parent-track context cannot be resolved', async () => {
    const storage = new Map<string, string>();
    const repository = new AsyncStoragePracticeRepository();
    ...
    await assert.rejects(
      repository.saveLoop({
        ...SAVED_LOOP,
        id: 'loop-missing-parent-track-context',
        sourceId: 'drive:missing-source',
        sourceName: '',
      }),
      /must preserve parent-track context/,
    );
  });
});
